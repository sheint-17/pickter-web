  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* 탭 */}
        <div style={{
          display: 'flex', background: '#F4F4F5', borderRadius: '10px',
        ...
        <button
          ...
          style={{
            ...
            background: '#FEE500', cursor: 'not-allowed',
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px',
        ...
        <p style={{ fontSize: '12px', color: '#9CA3AF', textAlign: 'center', margin: 0, lineHeight: 1.6 }}>
          {mode === 'signup'
            ? '가입 시 픽터의 이용약관 및 개인정보처리방침에 동의합니다'
            : '계정이 없으신가요?'}{' '}
          {mode === 'login' && (
            <button
              onClick={() => onModeChange('signup')}
              style={{ background: 'none', border: 'none', color: '#7B2FBE', fontWeight: 600, cursor: 'pointer', fontSize: '12px', padding: 0 }}
            >
              가입하기
            </button>
          )}
