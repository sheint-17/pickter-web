...
function IssueCreateForm() {
  const [state, formAction, isPending] = useActionState<CreateIssueState, FormData>(
    createIssue,
    null
  )
        ...
        <div style={{
          ...
          <FieldRow label="제목">
            <input
              name="title"
              type="text"
              placeholder="예: 트럼프, 2026년 안에 탄핵될까?"
              style={inputStyle}
              required
            />
          </FieldRow>
          ...
          <div style={{ display: 'flex', gap: '12px', marginBottom: '16px' }}>
            <div style={{ flex: 1 }}>
              <label style={labelStyle}>
                <span style={{ color: Colors.yes }}>픽</span> 선택지 label
              </label>
              <input
                name="yes_label"
                type="text"
            ...
            <div style={{ flex: 1 }}>
              <label style={labelStyle}>
                <span style={{ color: Colors.no }}>패스</span> 선택지 label
              </label>
              <input
                name="no_label"
                type="text"
                placeholder="예: 탄핵 안 된다"
                style={inputStyle}
                required
              />
            </div>
...
export default function AdminTabs({
  ...
  const [tab, setTab] = useState<'create' | 'settle' | 'review'>('create')
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
        ...
        <button style={tabStyle(tab === 'settle')} onClick={() => setTab('settle')}>
          이슈 정산
        </button>
      ...
      {tab === 'create' && <IssueCreateForm />}
      {tab === 'settle' && <AdminIssueList issues={issues} />}
      {tab === 'review' && <ProposalReviewList proposals={proposals} />}
    </div>
  )
}
